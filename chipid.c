...
bool chipid_check_serie(int fd, const struct _chip_serie* serie, struct _chip* chip)
{
	...
	for (uint32_t i = 0; i < serie->nb_chips; i++) {
		if ((serie->chips[i].cidr == (cidr & ~1)) && serie->chips[i].exid == exid) {
			*chip = serie->chips[i];
			chip->rev_char = 'A' + (cidr & 1);
			return true;
		}
	}
...
const struct _chip_serie* chipid_identity_serie(int fd, struct _chip* chip)
{
	for (int i = 0; i < ARRAY_SIZE(_chip_series); i++)
